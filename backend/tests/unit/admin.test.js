...
describe('Admin Model Unit Tests', () => {
  ...
  describe('Admin Static Methods', () => {
    test('findByEmail deve buscar por email', async () => {
      const mockFindOne = jest.spyOn(Admin, 'findOne');
      mockFindOne.mockResolvedValue({ email: 'teste@admin.com' });
      
      const result = await Admin.findByEmail('teste@admin.com');
      
      expect(mockFindOne).toHaveBeenCalledWith({ email: 'teste@admin.com' });
      expect(result.email).toBe('teste@admin.com');
      
      mockFindOne.mockRestore();
    });
    
    test('getActiveAdmins deve buscar apenas ativos', async () => {
      const mockFind = jest.spyOn(Admin, 'find');
      mockFind.mockReturnValue({ isActive: true });
      
      await Admin.getActiveAdmins();
  ...
  describe('Admin Update Operations', () => {
    test('deve atualizar updatedAt ao salvar', () => {
      const adminData = {
        name: 'Teste Admin',
        email: 'teste@admin.com',
        password: '123456'
  ...
  describe('Admin Validation Edge Cases', () => {
    test('deve aceitar email sem @ (Mongoose não valida formato)', () => {
      const adminData = {
        name: 'Teste Admin',
        email: 'emailinvalido',
        password: '123456'
      ...
      expect(error).toBeUndefined();
      expect(admin.email).toBe('emailinvalido');
    });
    
    test('deve aceitar email com domínio válido', () => {
      const adminData = {
        name: 'Teste Admin',
